...
/* ============================================================
 * IM3 cross-browser static icon layer v10.0
 * Uses local inline SVG data URIs for the React shell so the UI
 * does not show broken external icons when CDN/image hosts are blocked.
 * ============================================================ */
function im3xInlineSvgIcon(label, bg = "#113F67", fg = "#FFFFFF") {
  const safe = String(label || "IM").slice(0, 3).toUpperCase();
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" role="img" aria-label="${safe}">
    <rect x="3" y="3" width="42" height="42" rx="11" fill="${bg}"/>
    <circle cx="35" cy="13" r="5" fill="#F4D66F" opacity=".92"/>
    <path d="M14 31h20M14 24h20M14 17h20" stroke="${fg}" stroke-width="3" stroke-linecap="round" opacity=".92"/>
    <text x="24" y="37" text-anchor="middle" font-family="Arial, sans-serif" font-size="9" font-weight="700" fill="${fg}">${safe}</text>
  </svg>`;
  return "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(svg);
}

function im3xRefreshSvgIcon() {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" role="img" aria-label="Reload">
    <rect x="3" y="3" width="42" height="42" rx="11" fill="#147A86"/>
    <path d="M33.5 16.5A12 12 0 1 0 36 28" fill="none" stroke="#FFFFFF" stroke-width="3.8" stroke-linecap="round"/>
    <path d="M33.6 9.8v8.3h-8.3" fill="none" stroke="#F4D66F" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;
  return "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(svg);
}

const IM3_INLINE_ICON_LABELS = {
  moon:"MO", sun:"SU", refresh:"", loader:"LD", render:"PL", bell:"NO", cancel:"X", download:"DL", image:"IMG", report:"RP",
  rows:"TB", save:"SV", success:"OK", warning:"!", empty:"Ø", trash:"TR", close:"X", portfolio:"PF", factory:"PR",
  price:"$", revenue:"RV", capex:"CX", opex:"OX", tax:"TX", npv:"NPV", irr:"%", mcda:"MC", dynamics:"SD",
  score:"SC", decision:"DC", scenario:"SN", probability:"P", risk:"RK", file:"FL", category:"CT", location:"LC",
  timeline:"TL", company:"CO", handshake:"PA", calendar:"YR", trendUp:"UP", trendDown:"DN", currency:"FX",
  gauge:"GA", clock:"TM", growth:"GR", home:"HM", export:"EX", database:"DB", oil:"OG", market:"MK",
  calculator:"CA", payback:"PB", cashflow:"CF", government:"GV", law:"LW", security:"SE", esg:"ESG",
  technology:"TE", flow:"FW", engineering:"EN", verified:"✓", pause:"PA", expand:"↗", exit:"OUT", switch:"SW",
  checklist:"CL", scale:"WT", edit:"ED", recycle:"RE", community:"LC", innovation:"IN", sliders:"SL", tornado:"TO",
  statistics:"ST", fingerprint:"ID", formula:"Σ", list:"LS", api:"API", compare:"CP", pdf:"PDF", briefcase:"PK",
  diagnostics:"DG", dashboard:"DB"
};

Object.keys(IM3_ACTION_ICONS).forEach(function(key){
  IM3_ACTION_ICONS[key] = key === "refresh"
    ? im3xRefreshSvgIcon()
    : im3xInlineSvgIcon(IM3_INLINE_ICON_LABELS[key] || key.slice(0,2), key === "warning" ? "#B83A35" : key === "success" ? "#168A5B" : "#113F67");
});

const IM3_NAV_INLINE_ICON_BY_ID = { overview:"DB", input:"IN", analysis:"AN", graphs:"GR", reports:"RP", admin:"AD" };
IM3_NAV.forEach(function(item){
  item.icon = im3xInlineSvgIcon(IM3_NAV_INLINE_ICON_BY_ID[item.id] || item.label.slice(0,2), item.id === "reports" ? "#147A86" : "#113F67");
});

const IM3_LORDICONS = {
  dashboard:"https://cdn.lordicon.com/abwrkdvl.json",
  projects:"https://cdn.lordicon.com/iltqorsz.json",
  assumptions:"https://cdn.lordicon.com/lecprnjb.json",
  production:"",
  dcf:"https://cdn.lordicon.com/ikjaszcz.json",
  risk:"https://cdn.lordicon.com/akqsdstj.json",
  monteCarlo:"https://cdn.lordicon.com/kthelypq.json",
  reports:"https://cdn.lordicon.com/fpmskzsv.json",
  saving:"https://cdn.lordicon.com/tdrtiskw.json",
  calculating:"https://cdn.lordicon.com/xjovhxra.json",
  success:"https://cdn.lordicon.com/oqdmuxru.json",
  error:"https://cdn.lordicon.com/usownftb.json",
  empty:"https://cdn.lordicon.com/wxnxiano.json"
};
  ...
  return (
    <label className={className}>
      ...
      {editable && dropdownOptions.length ? (
        <select value={value || ""} onChange={e => onChange(e.target.value)}><option value="">Select…</option>{dropdownOptions.map((opt, idx) => <option key={idx} value={getOptionValue(opt)}>{im3xOptionLabel(opt) || getOptionLabel(opt)}</option>)}</select>
      ) : editable ? (
...
function SafeIcon({ src, fallback = IM3_ACTION_ICONS.dashboard, alt = "", className = "" }) {
  const [safeSrc, setSafeSrc] = useState(src || fallback || IM3_ACTION_ICONS.dashboard);
  useEffect(() => { setSafeSrc(src || fallback || IM3_ACTION_ICONS.dashboard); }, [src, fallback]);
  return <img className={`im3rx-safe-icon ${className}`} src={safeSrc} alt={alt} loading="lazy" decoding="async" onError={() => setSafeSrc(fallback || IM3_ACTION_ICONS.dashboard)} />;
}

function AnimatedIcon({ kind, fallback, mode = "hover", className = "", size = 24, forceFallback = false }) {
  const src = IM3_LORDICONS[kind] || "";
  const canUseLordicon = !forceFallback && src && window.customElements?.get?.("lord-icon");
  const trigger = mode === "loop" ? "loop" : "hover";
  return <span className={`im3rx-animated-icon ${className}`} data-mode={mode} data-kind={kind || ""}>
    {canUseLordicon ? <lord-icon src={src} trigger={trigger} delay={mode === "gentle" ? "1800" : "0"} colors="primary:#F9DA7B,secondary:#1F78B4" style={{width:`${size}px`,height:`${size}px`}}></lord-icon> : fallback ? <SafeIcon src={fallback} alt="" /> : null}
  </span>;
}
...
function App(){
 useEffect(()=>{
   function handleImageError(event){
     ...
     if (!img || img.tagName !== "IMG" || img.dataset.im3FallbackApplied === "1") return;
     img.dataset.im3FallbackApplied = "1";
     img.classList.add("im3rx-img-fallback-applied");
     img.src = IM3_ACTION_ICONS.dashboard || im3xInlineSvgIcon("IM");
   }
