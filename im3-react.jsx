...
function AnimatedIcon({ kind, fallback, mode = "hover", className = "", size = 24, forceFallback = false }) {
  ...
  return <span className={`im3rx-animated-icon ${className}`} data-mode={mode} data-kind={kind || ""}>
    {canUseLordicon ? <lord-icon src={src} trigger={trigger} delay={mode === "gentle" ? "1800" : "0"} colors="primary:#F9DA7B,secondary:#1F78B4" style={{width:`${size}px`,height:`${size}px`}}></lord-icon> : fallback ? <img src={fallback} alt=""/> : null}
  </span>;
}
