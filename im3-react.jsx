...
function ActivityDrawer({open,log,onClose,onClear}){return <aside className={`im3rx-activity ${open?"open":""}`}><div><strong>Execution history</strong><span className="im3rx-activity-actions"><button onClick={onClear} disabled={!log.length}>Clear</button><button onClick={onClose}>x</button></span></div>{!log.length?<p>No activity yet.</p>:log.map((item,idx)=><article key={idx} className={item.type}><span>{im3xDateTime(item.time)}</span><strong>{item.text}</strong></article>)}</aside>}
