...
function im3xWithTimeout(promise,ms,label){let timer;return Promise.race([promise,new Promise((_,reject)=>{timer=window.setTimeout(()=>reject(new Error((label||"Request timeout")+" after "+Math.round(ms/1000)+"s")),ms)})]).finally(()=>window.clearTimeout(timer));}
function im3xIsAndroid(){return /android|wv/i.test(String(navigator.userAgent||""));}
function im3xApiBudget(ms){return im3xIsAndroid()?Math.max(ms,260000):ms;}
function im3xDelay(ms){return new Promise(resolve=>window.setTimeout(resolve,ms));}
async function im3xRetry(fn,attempts,label){let last;for(let i=1;i<=attempts;i++){try{return await fn(i)}catch(e){last=e;if(i>=attempts)break;await im3xDelay(1200+i*900)}}throw last||new Error(label||"Request failed");}
...
function App(){
 ...
 if(loading||setupError)return <div className="im3rx-boot-screen">{setupError?<SetupError message={setupError} progress={progress}/>:<LoadingState text="Loading IM3 model data" progress={progress}/>}</div>;
