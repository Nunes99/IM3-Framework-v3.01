<!--
  IM³ Framework — React UI Shell v7.5
...
<div id="im3-react-root">
  <div class="im3rx-loading">
    <div></div>
    <strong>Loading IM³ React interface…</strong>
  </div>
</div>
...
(function installIM3ApiFallback(){
  ...
  const fallbackApi = {
    ...
    async generatePdf(type = "executive", projectId = "", language = "en", filters = {}){
      ...
      return apiCall(action, { projectId, language, filters: encodePayload(filters) }, 120000);
    },
    async generateInvestmentPack(projectId = "", language = "en", filters = {}){
      return apiCall("investmentpack", { projectId, language, filters: encodePayload(filters) }, 180000);
    },
  ...
  window.im3GeneratePdf = window.im3GeneratePdf || ((type, projectId, language, filters) => window.IM3Api.generatePdf(type, projectId, language, filters));
  window.im3GenerateInvestmentPack = window.im3GenerateInvestmentPack || ((projectId, language, filters) => window.IM3Api.generateInvestmentPack(projectId, language, filters));
...
<script type="text/babel" src="./im3-react.jsx"></script>